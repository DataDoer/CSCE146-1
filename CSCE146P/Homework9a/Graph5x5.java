...
public class Graph5x5 extends AdjMatrix {
  ...
  public void generateWords(){
    for(int i = 0; i < size; i++) {
      ...
      traverse(i, new ArrayList<Integer>());
    }
  }

  public void traverse(int spot, ArrayList<Integer> travelled) {
    travelled.add(spot);
    String word = getWord(travelled);

    if(dictionary.contains(word)) wordsfound.add(word);
    if(travelled.size() >= maxlength) return;
    for(int i = 0; i < size; i++) {
      if(linked(spot, i) && !travelled.contains(i)) {
        ArrayList<Integer> newtravelled = super.deepCopyArrayList(travelled);
        traverse(i,newtravelled);
      }
    }
  }
  ...
  public String getWord(ArrayList<Integer> travelled) {
    String s = "";
    for(int a : travelled) {
      s += getletter(a);
    }
